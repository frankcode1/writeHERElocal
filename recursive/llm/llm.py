...
class OpenAIApiProxy():
    ...
    def _load_local_qwen(self):
        ...
        try:
            ...
            raise RuntimeError(
                "Local Qwen inference requires transformers and torch. "
                "Install them with `pip install transformers torch accelerate`."
            ) from exc

        device = os.getenv("LOCAL_QWEN_DEVICE", "cuda:0" if torch.cuda.is_available() else "cpu")
        logger.info(f"Loading local Qwen model from {model_path} on {device}")
    ...
    def _call_local_qwen(self, model, messages, temperature=None, **kwargs):
        with _LOCAL_QWEN_LOCK:
            ...
            if hasattr(tokenizer, "apply_chat_template"):
                ...
                if os.getenv("LOCAL_QWEN_ENABLE_THINKING", "false").lower() in ("0", "false", "no"):
            ...
            if inputs["input_ids"].shape[-1] >= max_input_tokens:
                logger.warning(
                    f"Local Qwen prompt was truncated to {max_input_tokens} tokens. "
                    "Set LOCAL_QWEN_MAX_INPUT_TOKENS to change this limit."
                )
            ...
            try:
                ...
                logger.error(f"Error with local Qwen inference: {exc}")
                raise
    ...
    def call(self, model, messages, no_cache = False, overwrite_cache=False, tools=None, temperature=None, headers={}, use_official=None, **kwargs):
        ...
        is_local_qwen = is_local_qwen_model(model)
        ...
        if is_local_qwen:
            local_generation_args = copy.deepcopy(params_gpt)
            local_generation_args.pop("model", None)
            local_generation_args.pop("messages", None)
            local_generation_args.pop("temperature", None)
            result = self._call_local_qwen(model, messages, temperature=temperature, **local_generation_args)
            if not no_cache:
