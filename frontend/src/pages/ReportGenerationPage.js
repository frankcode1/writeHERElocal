...
const commonModels = [
  { label: 'Local Qwen 4B', value: 'qwen4b' },
...
const ReportGenerationPage = () => {
  ...
  const [model, setModel] = useState('qwen4b');
  ...
  const handleSubmit = async (e) => {
    ...
    const isLocalModel = model.toLowerCase().includes('qwen') || model.toLowerCase().startsWith('local');
    
    if (!isLocalModel && isOpenAIModel && !apiKeys.openai) {
      setError('Please provide your OpenAI API key in the settings section.');
      setShowApiSection(true);
      return;
    }
    
    if (!isLocalModel && isClaudeModel && !apiKeys.claude) {
      setError('Please provide your Anthropic Claude API key in the settings section.');
      setShowApiSection(true);
      return;
    }
    
    if (!isLocalModel && isGeminiModel && !apiKeys.gemini) {
